  ...
  test("data product test case") {
    val res = scala.util.Try {
      val loader = SchemaLoader.builder
        .schemaJson(
          new JSONObject(new JSONTokener(getClass.getClassLoader.getResourceAsStream("dataproduct.schema.json")))
        )
        .draftV6Support
        .build()
      ...
      val (nd, allD: Map[ComparableType, SimplifiedDef]) = res.get
